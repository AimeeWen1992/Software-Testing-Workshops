...
void integrationTest()
{
	...
	int valid, addSucces;
	// Check how big size of descriotions array
	int numProducts = 6;

	// Test scenario: Adding items to the cart
	// Simulate the user entering product names to add to the cart and validate the results
	printf("Integration Test - Adding Items to Cart\n");

	// Test case: e.g. Adding a product that does not exist
	strcpy(productName, "bread");
	valid = findString(productName, descriptions, numProducts);
	addSucces = add2Cart(&cart, valid);
	if (!addSucces && valid > 0)
	{
		printf("Test Passed: Product '%s' added to the cart as expected.\n",productName);
	}
	else
	{
		printf("Test Failed: Product '%s' incorrectly added to the cart.\n",productName);
	}
}

int main(void)
{
	// Other code in the main function
